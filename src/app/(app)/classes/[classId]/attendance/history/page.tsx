...
import AttendanceHistoryClientPage from './client-page';
...
export default function AttendanceHistoryPage({ params }: { params: { classId: string } }) {
  const classId = params.classId as keyof typeof classDetailsData;
  const className = classDetailsData[classId]?.name || 'Unknown Class';

  return <AttendanceHistoryClientPage classId={classId} className={className} />;
}
